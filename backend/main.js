...
app.get('/api/register', (req,res) => {
    ...
    if(dec.email && dec.username && dec.password && dec.confirmpassword && dec.token)
    {
        if(dec.token !== process.env.SECRET_TOKEN_CODE){
            var message = {data:'no access to api'}
            var cipher = cryptoJs.AES.encrypt(JSON.stringify(message),process.env.SECRET_TOKEN_CODE).toString();
                    ...
                    key_check = () => {
                        ...
                        connection.query(CHECK_TOKEN,(err,result) => {
                            ...
                            if(result[0]){
                                ...
                                console.log(token)
                                console.log(result)
                                const ADD_USER_TO_DB = `INSERT INTO users(username,password,email,token,expire) VALUES('${dec.username}','${dec.password}','${dec.email}','${token}','june')`
...
app.get('/api/getvideobyuid',(req,res)=>{
    ...
    connection.query(GET_VIDEO,(err,result)=>{
        ...
        if(result[0])
        {
            var a = result[0].id
            var b = result[0].thumext
            var c = result[0].videoext
            var bitmap = fs.readFileSync(`./videos/${a}.${b}`)
            var thum = new Buffer(bitmap).toString('base64');
            bitmap = fs.readFileSync(`./videos/${a}.${c}`)
            var video = new Buffer(bitmap).toString('base64')
            var message = {data:result}
            var cipher = cryptoJs.AES.encrypt(JSON.stringify(message),process.env.SECRET_TOKEN_CODE).toString();
            cipher = cipher.replace(/[/]/g,'★')
            cipher = cipher.replace(/[+]/g,'☆')
            var obj = {video:video,thum:thum,data:cipher}
            return res.json({data : obj})
        }
    })
})
...
app.get('/api/search',(req,res)=>{
    ...
    let SEARCH = `SELECT * FROM videos WHERE videoname LIKE '%${input}%'`
...
app.post('/api/upload', (req,res) => {
    ...
    if(dec.token !== process.env.SECRET_TOKEN_CODE)
    {
        return res.json({data:"no access to api"})
    }
